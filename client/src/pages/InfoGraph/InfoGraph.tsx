...
const getStatusBadge = (status?: string, hasOutput?: boolean) => {
  ...
  // If status is missing but we still don't have output, treat it as in-progress.
  if (!s && !hasOutput) {
    return { label: 'In progress', bg: 'rgba(99,102,241,0.14)', border: 'rgba(99,102,241,0.35)', color: '#A78BFA', running: true };
  }
...
function ImageGeneratorToolUI(props: ToolUIProps) {
  ...
  // Requested behavior:
  // - If imageUrl is undefined, keep showing loading (even if tool status says output-available)
  ...
  return (
    <>
          ...
          <div style={{
            ...
            <div style={{ textAlign: 'center', maxWidth: '520px' }}>
              <h3 style={{ margin: 0, fontSize: '24px', fontWeight: 600, color: '#FFFFFF' }}>
                Open the chat to build your infographic
              </h3>
              <p style={{ marginTop: '12px', color: '#888', fontSize: '14px', lineHeight: '1.6' }}>
                Each chat keeps its own infographic. Ask the assistant to create slides with images and titles.
              </p>
            </div>
          </div>
        ) : (
          <PresentationDisplay mode="infographic" slides={presentationState.slides} />
        )}
      </div>
      ...
      <HsafaChat
        ...
        placeholder="Describe the infographic you want…"
        primaryColor="#6366F1"
        accentColor="#8B5CF6"
        alwaysOpen={true}
        expandable={false}
        ...
        presetPrompts={[
          {
            label: "🧩 Build a 3-slide infographic",
            prompt: "Create a 3-slide infographic with: 1) a title slide, 2) key metrics slide, 3) a conclusion slide. Each slide should have an image and a clear title."
          },
          {
            label: "🖼️ Regenerate images",
            prompt: "Regenerate the slide images with a more modern, clean design."
          },
          {
            label: "👁️ Show slides",
